package study.sparkstreaming.v2
      ...
      lines.foreachPartition(partition =>
        partition.foreach(x => {
          ...
          for (i <- 0 until sorted.size) {
            ...
            println(s"数据更新开始...........")
            val set: ResultSet = stat.executeQuery(s"select * FROM `spark_test` WHERE `field` = '${field}'")
            if(set.next()){
              println(s"数据更新循环")
              val dbfield: String = set.getString("field")
              val dbtimes: Int = set.getInt("times")
              println(s"数据库查出数据为:dbfield=${dbfield},dbtimes=${dbtimes}")
              val resUpdate = stat.executeUpdate(s"UPDATE `spark_test` SET `times` = '${times+dbtimes}'  WHERE `field` = '${field}'")
            }else {
              println(s"数据入库开始...........")
              val result: Int = stat.executeUpdate(s"INSERT INTO `spark_test`( `create_time`,`field`, `times`) VALUES ( '${pool.currentTime()}','${field}', ${times})")
            }
            stat.close()
            conn.close()
          }
