...
object AlsArithmeticPractice {
  def main(args: Array[String]) {
    ...
    // 设置运行环境
    val conf = new SparkConf().setAppName("ALSPractice").setMaster("local[*]")
    val sc = new SparkContext(conf)

    val personalRatingsPath = "D:\\mydata\\ml\\dt\\ml-1m\\personalRatings.txt"
    val moviesPath = "D:\\mydata\\ml\\dt\\ml-1m\\movies.dat"
    val ratingsPath = "D:\\mydata\\ml\\dt\\ml-1m\\ratings.dat"
    val userPath = "D:\\mydata\\ml\\dt\\ml-1m\\users.dat"
    val modelPath = "D:\\mydata\\ml\\dt\\ml-1m\\model"
    val outPath = "D:\\mydata\\ml\\dt\\ml-1m\\recommendations"
    ...
    println("==================用户评分数据===================")
    //Stream(Rating(0,780,4.0), Rating(0,590,3.0), Rating(0,1216,4.0), Rating(0,648,5.0), Rating(0,344,3.0)
    ...
    //-测试(20%)
    val numPartions = 4
    //通过key(10的余数，均衡分布，所以x._1 < 6基本能够切分出大约60%的数据量) ---union 合并不去重
